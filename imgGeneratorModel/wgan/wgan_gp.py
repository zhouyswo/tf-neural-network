...
# generate mnist style image by wgan-gp
    ...
    for i in range(epoch_num):
        for j in range(batch_num):
            ...
            for k in range(d_iters):
                ...
                train_alpha = np.random.uniform(0.0, 1.0, size=[batch_size, 1, 1, 1]).astype(np.float32)
                # Update the discriminator
                _, dLoss = sess.run([trainer_d, d_loss],
                                    feed_dict={random_input: train_noise, real_image: train_image, alpha: train_alpha,
                                               is_train: True})
