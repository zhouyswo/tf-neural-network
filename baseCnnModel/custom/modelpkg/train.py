  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    for epoch in xrange(start_epoch, start_epoch + args.num_epochs):
      ...
      for step in xrange(args.num_batches):
        ...
        _, loss_value, top1_accuracy = sess.run(
            [train_op, cross_entropy_mean, top1acc], options= run_options, run_metadata= run_metadata)
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--crop_size', nargs= 2, default= [200,200], type= int, action= 'store', help= 'The width and height of images after random cropping')
    parser.add_argument('--batch_size', default= 4, type= int, action= 'store', help= 'The training batch size')
    ...
    parser.add_argument('--num_epochs', default= 20, type= int, action= 'store', help= 'The number of epochs')
    ...
    parser.add_argument('--LR_steps', type=int, nargs='+', default=[3, 5, 7, 10], help='LR change epochs')
    # parser.add_argument('--LR_values', type=float, nargs='+', default=[0.01, 0.005, 0.001, 0.0005, 0.0001], help='LR change epochs')
    parser.add_argument('--LR_values', type=float, nargs='+', default=[0.012, 0.005, 0.001, 0.0005, 0.0001], help='LR change epochs')
    ...
    # Counting Total number of training examples
    ...
    # rebuild logging directory if necessary
